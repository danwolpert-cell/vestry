...
function HedgeFunds(){const [search,setSearch]=useState("");const [sortBy,setSortBy]=useState("rank");const [sel,setSel]=useState(null);const filtered=HEDGE_FUNDS.filter(f=>f.name.toLowerCase().includes(search.toLowerCase())||f.manager.toLowerCase().includes(search.toLowerCase())||f.country.toLowerCase().includes(search.toLowerCase())||f.strategy.toLowerCase().includes(search.toLowerCase())).sort((a,b)=>sortBy==="aum"?b.aum-a.aum:sortBy==="return"?b.annualReturn-a.annualReturn:a.rank-b.rank);const sc=s=>({"Global Macro":"#f59e0b","Quantitative":"#4f8eff","Multi-Strategy":"#a855f7","Long/Short Equity":"#10b981","Event Driven":"#f43f5e","Value":"#06b6d4","Credit":"#ec4899","Activist":"#ff6b35","Sovereign Wealth":"#fbbf24","Private Equity":"#8b5cf6","Global":"#22d3ee"}[s]||"#7c87a0");return(<div style={{maxWidth:1140,margin:"0 auto",padding:"28px 24px"}}>{sel&&(<div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.78)",zIndex:200,display:"flex",alignItems:"center",justifyContent:"center"}} onClick={()=>setSel(null)}><div style={{background:C.card,border:"1px solid "+C.border,borderRadius:16,padding:32,width:460}} onClick={e=>e.stopPropagation()}><div style={{display:"flex",justifyContent:"space-between",marginBottom:20}}><div><div style={{fontSize:20,fontWeight:800,color:C.text}}>{sel.name}</div><div style={{color:C.sub,fontSize:13,marginTop:2}}>Founded {sel.founded} - {sel.country}</div></div><button onClick={()=>setSel(null)} style={{background:"none",border:"none",color:C.sub,cursor:"pointer",fontSize:20}}>X</button></div><div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:16}}>{[["Manager",sel.manager],["Strategy",sel.strategy],["AUM","$"+sel.aum+"B"],["Annual Return","+"+sel.annualReturn+"%"],["Rank","#"+sel.rank],["Founded",sel.founded]].map(([l,v])=>(<div key={l} style={{background:C.surface,borderRadius:8,padding:"10px 14px"}}><div style={{color:C.sub,fontSize:11,marginBottom:3}}>{l}</div><div style={{color:C.text,fontWeight:700,fontSize:14}}>{v}</div></div>))}</div><span style={{fontSize:12,fontWeight:700,padding:"4px 12px",borderRadius:6,background:sc(sel.strategy)+"22",color:sc(sel.strategy)}}>{sel.strategy}</span></div></div>)}<div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:20}}><div><div style={{fontWeight:800,fontSize:22,color:C.text}}>Top 30 Hedge Funds</div><div style={{color:C.sub,fontSize:13,marginTop:2}}>Global rankings - Click any fund for details</div></div><select value={sortBy} onChange={e=>setSortBy(e.target.value)} style={{background:C.card,border:"1px solid "+C.border,borderRadius:8,color:C.text,padding:"8px 12px",fontSize:13,cursor:"pointer"}}><option value="rank">Sort: Rank</option><option value="aum">Sort: AUM</option><option value="return">Sort: Return</option></select></div><input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Search name, manager, country or strategy..." style={{width:"100%",background:C.card,border:"1px solid "+C.border,borderRadius:10,padding:"12px 16px",color:C.text,fontSize:14,outline:"none",marginBottom:20,boxSizing:"border-box"}}/><div style={{background:C.surface,border:"1px solid "+C.border,borderRadius:14,overflow:"hidden"}}><div style={{display:"grid",gridTemplateColumns:"50px 1.8fr 110px 150px 90px 110px",padding:"10px 20px",color:C.sub,fontSize:11,textTransform:"uppercase",letterSpacing:"0.06em",borderBottom:"1px solid "+C.border}}>{["#","Fund","Country","Strategy","AUM","Return"].map((h,i)=><span key={i} style={{textAlign:i>1?"right":"left"}}>{h}</span>)}</div>{filtered.map(f=>(<div key={f.rank} onClick={()=>setSel(f)} style={{display:"grid",gridTemplateColumns:"50px 1.8fr 110px 150px 90px 110px",padding:"13px 20px",borderBottom:"1px solid "+C.border,cursor:"pointer",alignItems:"center",transition:"background 0.1s"}} onMouseEnter={e=>e.currentTarget.style.background=C.card} onMouseLeave={e=>e.currentTarget.style.background="transparent"}><div style={{color:C.sub,fontWeight:700,fontSize:12}}>#{f.rank}</div><div><div style={{fontWeight:700,fontSize:13,color:C.text}}>{f.name}</div><div style={{color:C.sub,fontSize:11,marginTop:1}}>{f.manager}</div></div><div style={{textAlign:"right",color:C.text,fontSize:12}}>{f.country}</div><div style={{textAlign:"right"}}><span style={{fontSize:11,fontWeight:700,padding:"2px 7px",borderRadius:5,background:sc(f.strategy)+"22",color:sc(f.strategy)}}>{f.strategy}</span></div><div style={{textAlign:"right",fontWeight:700,color:C.text,fontSize:13}}>${f.aum}B</div><div style={{textAlign:"right",fontWeight:700,color:C.green,fontSize:13}}>+{f.annualReturn}%</div></div>))}</div></div>);}
